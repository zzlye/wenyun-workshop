...
export default function AccountLoginModal({ open, onClose }: AccountLoginModalProps) {
  ...
  const handleAccountLogin = async () => {
    ...
    try {
      ...
      updateAccountSession(session, { accountApiKeyMode: normalizedSettings.profiles.find((profile) => profile.id === LOCKED_WENYUN_PROFILE_ID)?.apiKey.trim() ? normalizedSettings.accountApiKeyMode : 'account' })
      setAccountPassword('')
